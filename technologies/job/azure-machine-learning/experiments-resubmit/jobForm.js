...
/**
 * Function to retrieve experiments runs linked to selected experiment
 * @param {Object} entity - Contains entity data including featuresValues.
 * @param {Object} entity.featuresValues - Contains all the values from the entity features declared in the context.yaml
 */
exports.getExperimentsRuns = async ({ featuresValues }) => {
  try {
    const apiUrl = await getExperimentsApiServer(featuresValues.workspace);

    const res = await axios.get(
      `${apiUrl}/history/v1.0/subscriptions/${featuresValues.endpoint.subscriptionId}/resourceGroups/${featuresValues.resourceGroup.label}/providers/Microsoft.MachineLearningServices/workspaces/${featuresValues.workspace.label}/experimentids/${featuresValues.experiment.id}/runs?$filter=(runType%20eq%20azureml.scriptrun)`,
      await getHeadersWithAccessTokenForManagementResource(featuresValues.endpoint)
    );

    if (checkDataFromAzureResponse(res)) {
      const experimentsRuns = res.data.value;

      if (experimentsRuns.length > 0) {
        return Response.success(experimentsRuns.map(({ runId, runNumber }) => ({
          id: runId,
          label: `Run ${runNumber} - Run ID : ${runId}`,
        })));
      }
    }
