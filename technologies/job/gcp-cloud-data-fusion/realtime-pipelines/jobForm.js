...
/**
 * Function to retrieve real-time pipelines linked to the selected instance.
 * @param {Object} entity - Contains entity data including featuresValues.
 * @param {Object} entity.featuresValues - Contains all the values from the entity features declared in the context.yaml
 */
exports.getRealtimePipelines = async ({ featuresValues }) => {
  const gcpKey = JSON.parse(featuresValues.endpoint.jsonKey);

  try{
    const { data } = await axios.get(
      `${featuresValues.instance.apiEndpoint}/v3/namespaces/default/apps?artifactName=cdap-data-streams`,
      await getHeadersWithAccessToken(gcpKey),
    );

    if (!data || !data.length) {
      return Response.empty('No real-time pipelines availables')
    }
    ...
    return getErrorMessage(error, "Can't retrieve real-time pipelines");
  }
};
