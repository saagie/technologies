  ...
  try {
    ...
    if (userData && userData.icSessionId && userData.serverUrl) {
      const result = await axios.post(
        ...
        {
          '@type': 'job',
          taskId: (job.featuresValues.task && job.featuresValues.task.id) || (job.featuresValues.workflow && job.featuresValues.workflow.id),
          taskType: (job.featuresValues.task && job.featuresValues.task.type) || WORKFLOW_TYPE
        },
