...
/**
 * Function to retrieve Pipelines runs for the selected Data Factory factory
 * @param {Object} entity - Contains entity data including featuresValues.
 * @param {Object} entity.featuresValues - Contains all the values from the entity features declared in the context.yaml
 */
exports.getPipelinesRuns = async ({ featuresValues }) => {
  try {
    const res = await axios.post(
      `${AZURE_MANAGEMENT_API_URL}${featuresValues.resourceGroup.id}/providers/Microsoft.DataFactory/factories/${featuresValues.factory.label}/queryPipelineRuns?api-version=2018-06-01`,
      {
    ...
    if (checkDataFromAzureResponse(res)) {
      const pipelinesRuns = res.data.value;

      if (pipelinesRuns.length > 0) {
        return Response.success(pipelinesRuns.map(({ runId, runStart, pipelineName }) => ({
          id: runId,
