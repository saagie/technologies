    ...
    with utils.SaagieUtils() as saagie_utils:
        ...
        for project in project_list:
            ...
            all_jobs = [{
                ...
                'cronScheduling': job["cronScheduling"],
            ...
            all_pipelines = [{
                ...
                'orchestration_id': pipeline["id"],
                'orchestration_name': pipeline["name"],
                'creation_date': pipeline["creationDate"],
                'instance_count': pipeline["pipelineInstanceCount"],
                'isScheduled': pipeline["isScheduled"],
                'cronScheduling': pipeline["cronScheduling"],
                'scheduleTimezone': pipeline["scheduleTimezone"]
            } for pipeline in pipeline_list]
            database_utils.supervision_saagie_pipelines_to_pg(all_pipelines)
            ...
            for pipeline in pipeline_list:
                if (pipeline["isScheduled"] and pipeline["pipelineInstanceCount"] > 0):
                    cron = croniter.croniter(pipeline["cronScheduling"], now)
                    ...
                    while (next_run < (datetime.now(tz=pytz.timezone("Europe/Paris"))+ relativedelta(months=1))):
                        ...
                        all_pipelines.append({
                            ...
                            'orchestration_type': "pipeline",
                            'orchestration_name': pipeline["name"],
                            'next_orchestration_id': i,
                            'next_instance_start': next_run,
                            'next_instance_end': next_run + timedelta(0,avg),
                            'avg_instance_duration': avg
                        })
            ...
            for job in job_list:
                if (job["isScheduled"] and job["countJobInstance"] > 0):
                    cron = croniter.croniter(job["cronScheduling"], now)
            ...
            database_utils.supervision_saagie_predict_to_pg(all_jobs)
