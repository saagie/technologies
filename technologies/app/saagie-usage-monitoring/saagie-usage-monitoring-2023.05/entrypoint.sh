...
if [ "$SAAGIE_PG_HOST" == "localhost" ]; then
  ...
  if [ "$(ls -A $PG_DATA_DIR)" ]; then
    ...
    su postgres -c "export PATH=$PATH:/usr/lib/postgresql/14/bin && pg_ctl -D ${PG_DATA_DIR} start" > /dev/null
  else
    ...
    {
      su postgres -c "/usr/lib/postgresql/14/bin/initdb -D $PG_DATA_DIR"
      su postgres -c "export PATH=$PATH:/usr/lib/postgresql/14/bin && pg_ctl start -D $PG_DATA_DIR"
