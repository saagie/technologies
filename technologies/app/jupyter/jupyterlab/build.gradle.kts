...
tasks.withType(com.bmuschko.gradle.docker.tasks.image.DockerBuildImage::class) {
    dependsOn(":jupyterlab-base-4.0.5:testImage")
    this.buildArgs.put(
        "BASE_CONTAINER",
    ...
    this.buildArgs.put(
        "PYTHON311_IMG",
        "saagie/python:3.11-1.181.0_SDKTECHNO-235"
    )
    this.buildArgs.put(
        "PYTHON310_IMG",
        "saagie/python:3.10-1.155.0"
    )
    this.buildArgs.put(
        "PYTHON39_IMG",
        "saagie/python:3.9-1.155.0"
    )
    this.buildArgs.put(
        "PYTHON38_IMG",
        "saagie/python:3.8-1.155.0"
    )
}
