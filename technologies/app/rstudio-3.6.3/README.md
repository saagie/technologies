...
This Docker image is available on [Saagie's DockerHub](https://hub.docker.com/r/saagie/rstudio) is based on the official [rocker/tidyverse:3.6.3](https://hub.docker.com/r/rocker/tidyverse/) image.
...
```
./gradlew :rstudio-3.6.3:buildImage
```

If you want to test the image, you can run:
```
./gradlew :rstudio-3.6.3:testImage
```
...
```
docker build -t saagie/rstudio:3.6.3 .
```
...
Run: 
```
docker run --rm -it -p 10087:8787 --name rstudio -e SAAGIE_BASE_PATH=/ -e PASSWORD=yourPassword saagie/rstudio:3.6.3
```
...
```
docker run --rm -it -p 10087:8787 -v $PWD/rstudio:/home -v $PWD/hadoop/conf:/etc/hadoop/conf --name rstudio -e SAAGIE_BASE_PATH=/ -e PASSWORD=yourPassword saagie/rstudio:3.6.3
```
