...
# [SAAGIE] Force WEB PROXY HOST REPLACEMENT
...
case ${AUTH} in
    ...
    *)
        if [ ! -z "${NIFI_WEB_PROXY_HOST}" ]; then
            echo 'NIFI_WEB_PROXY_HOST was set ('$NIFI_WEB_PROXY_HOST') but NiFi is not configured to run in a secure mode.  Will not update nifi.web.proxy.host.'
        fi
        ;;
esac

# [SAAGIE] Display configurations
echo ""
echo "******************************************"
echo " => /opt/nifi/nifi-current/conf/nifi.properties"
...
echo ""
echo "******************************************"
echo " => /root/.nifi-cli.nifi.properties"
cat /root/.nifi-cli.nifi.properties
echo ""
echo "******************************************"
echo ""
echo ""
